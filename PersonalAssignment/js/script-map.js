...
async function openRestaurantDailyMenu(restaurant){
    ...
    if(dailyFoodCourses.length == 0){
        ...
        const noMenuTextContent = 'No Daily menu found.'

        const foodItem = document.createElement('div');
        foodItem.classList.add('daily-item-none');

        const noMenuText = document.createElement('p');
        noMenuText.textContent = noMenuTextContent;

        foodItem.appendChild(noMenuText);
...
async function openRestaurantWeeklyMenu(restaurant){
    ...
    if (!WeeklyFoodCourses || WeeklyFoodCourses.length == 0) {
        const noMenuTextContent = `No weekly menu available for ${restaurant.name}`
        //console.log(noMenuTextContent);
        const foodItem = document.createElement('div');
        foodItem.classList.add('weekly-item-none');
